...
Para cada vista se ejecutó su consulta de contraste de [referencias_vistas.sql](referencias_vistas.sql): **EXCEPT ALL en ambos sentidos dio cero**, y la comparación completa de filas con el mismo ORDER BY dio igualdad. EXCEPT ALL detecta diferencias de multiplicidad; el orden pertenece a la consulta consumidora, no se garantiza por CREATE VIEW. Las tres consultas constituyen la solución de contraste de la Parte B; su elaboración asistida se registra en la DUIA.

Las consultas de contraste acceden directamente a las tablas base. Para V1, una CTE selecciona primero las categorías activas y luego se une con los productos activos: conserva ambos filtros de vigencia. Para V2, una CTE proyecta únicamente id, nombre y apellido del cliente antes de unir sus pedidos, sin excluir estados. Para V3, una subconsulta escalar obtiene el nombre del producto: la PK garantiza a lo sumo una coincidencia y la FK no nula del detalle garantiza que existe, por lo que coincide con el INNER JOIN de la vista. El subtotal usa el mismo precio histórico y cantidad; no se reemplaza por el precio actual.
