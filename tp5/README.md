...
Nuestro esquema usa `cliente` en vez de `usuario` y no contiene contraseñas. La vista de seguridad oculta los datos de contacto existentes y se prueba con un rol sin acceso a las tablas. No se agrega una columna de contraseña para simular el ejemplo. Se empleó Codex, declarando el uso real. La Parte B incluye las tres consultas de contraste y la comprobación de equivalencia, tanto sobre la base masiva como con casos límite.
